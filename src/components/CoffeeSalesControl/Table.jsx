...
const Table = ({ventas}) => {
      ...
      const getCafeClass = (tipoCafe) => {
            switch (tipoCafe) {
                  case "CAFÉ VERDE":
                    return "bg-[#1A4D2E] text-[#F4E3C0]"; // Verde profesional con texto beige crema
                  case "CAFÉ SECO":
                    return "bg-[#E0D2B8] text-[#3F3F3F]"; // Beige suave con texto gris profesional
                  case "PASILLA":
                    return "bg-[#1B1B1B] text-[#F4E3C0]"; // Negro sofisticado con texto beige crema
                  default:
    ...
    <div className="table  sm:table-auto size-full border-separate border-spacing-0 table-sm sm:table-sm md:table-sm lg:table-md ">
          ...
          <tbody className="text-[#3F3F3F]">
                {elementosActuales.map((e) => (
                      <>
                            <tr key={e.id_venta}>
                                  <th className="w-auto border-b font-semibold border-gray-300 text-xs md:text-sm">
                                        {transformarFecha(new Date(e.fecha))}
                                  </th>
                                  <td className="border-b border-gray-300 text-xs md:text-sm">
                                        {e.cantidad}
                                  </td>
                                  <td className="border-b border-gray-300 text-xs md:text-sm">
                                        {calcularCargas(e.cantidad)}
                                  </td>
                                  <td className="border-b border-gray-300 font-semibold  text-md md:text-lg">
                                        {convertirAPesosColombiano(e.valor)}
                                  </td>
                                 
                                  <td className="text-center border-b border-gray-300 ">
                                    <span className={`mt-1 px-2 py-1 text-xs md:text-sm rounded-lg  font-semibold ${getCafeClass(e.tipo_cafe)}`}>
                                       {e.tipo_cafe}
                                   </span>
                                  </td>      
                            </tr>
